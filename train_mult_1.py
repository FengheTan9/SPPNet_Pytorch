...
TRAIN_IMG_DIR = r'D:\transfer_learning_ds\office31\webcam'
VAL_IMAGE_DIR = r'D:\transfer_learning_ds\office31\webcam'
...
if __name__ == '__main__':
    ...
    # create model
    sppnet = SPPNet(gpu_num=len(DEVICE_IDS), batch=BATCH_SIZE, num_classes=31)
    ...
    dataloader_224, dataloader_180 = creat_imagenet_train_dataset(train_dir=TRAIN_IMG_DIR, batch=BATCH_SIZE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch % 2 == 0:
            ...
            sppnet.train()
            time_1 = time.time()
            for imgs, classes in dataloader_180:
                imgs, classes = imgs.to(device), classes.to(device)
                optimizer.zero_grad()
                # calculate the loss
                output = sppnet(imgs).to(device)
                loss = nn.CrossEntropyLoss()(output, classes)
                loss_fn_180.update(loss.item(), imgs.size(0))
                # update the parameters
                loss.backward()
                optimizer.step()
                total_steps += 1
            print('180-Epoch: {} \t loss: {} \t avg:{} \t time:{}'.format(epoch + 1, loss_fn_180.val, loss_fn_180.avg,
                                                                          time.time() - time_1), flush=True)
        lr_scheduler.step()
