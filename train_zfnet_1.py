...
class Net(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(1000, 31)
    def forward(self, x):
        return self.linear(x.net(x))
...
if __name__ == '__main__':
    ...
    # create model
    net = ZFNet(gpu_num=len(DEVICE_IDS), batch=BATCH_SIZE, num_classes=31)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        evalute(net, dataloader_val, epoch)
        lr_scheduler.step()

        if epoch > 50:
            # save checkpoints
            checkpoint_path = os.path.join(CHECKPOINT_DIR, 'zfnet_states_e{}.pkl'.format(epoch + 1))
            state = {
                'epoch': epoch,
